...
function buildSystemPrompt(tools: Map<string, BaseTool>, model: string, options?: AgentOptions): string {
  ...
  const parts = [
    ...
    "",
    "## Auto-Memory",
    "Use the Memory tool to automatically save important information for future conversations:",
    "- User preferences (language, frameworks, code style, communication preferences)",
    "- Project context (architecture decisions, tech stack, key file locations)",
    "- User feedback (if user corrects you or praises an approach, remember it)",
    "- External references (API docs, internal wikis, Slack channels)",
    "Save proactively but not excessively. One clear memory is better than five vague ones.",
